...
resource "vsphere_virtual_machine" "controller" {
  for_each = {for vm in var.controller_vm: vm.hostname => vm}


  name             = each.value.vmname #var.server1_vm_params["name"]
  num_cpus         = var.controller_vm_params["vcpu"]
  memory           = var.controller_vm_params["ram"]
  ...
  clone {
    ...
    customize {
      linux_options {
        ...
        domain    = var.controller_network_params["domain"]
      }

      network_interface {
        ipv4_address    = each.value.ipv4_address                       #var.server1_network_params["ipv4_address"]
        ipv4_netmask    = var.controller_network_params["prefix_length"]
      }

      ipv4_gateway = var.controller_network_params["gateway"]
      dns_server_list = var.dns_servers
      dns_suffix_list = ["${var.controller_network_params["domain"]}"]
    }
  }
