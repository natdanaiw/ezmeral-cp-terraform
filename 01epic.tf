...
resource "vsphere_virtual_machine" "epic" {
  depends_on = ["vsphere_virtual_machine.controller","vsphere_virtual_machine.gateway","vsphere_virtual_machine.k8snode"]

  for_each = {for vm in var.epic_vm: vm.hostname => vm}


  name             = each.value.vmname #var.server1_vm_params["name"]
  num_cpus         = var.epic_vm_params["vcpu"]
  memory           = var.epic_vm_params["ram"]
  ...
  clone {
    ...
    customize {
      linux_options {
        ...
        domain    = var.epic_network_params["domain"]
      }

      network_interface {
        ipv4_address    = each.value.ipv4_address                       #var.server1_network_params["ipv4_address"]
        ipv4_netmask    = var.epic_network_params["prefix_length"]
      }

      ipv4_gateway = var.epic_network_params["gateway"]
      dns_server_list = var.dns_servers
      dns_suffix_list = ["${var.epic_network_params["domain"]}"]
    }
  }
